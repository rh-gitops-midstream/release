...
# `operator-sdk run bundle`, bypassing CatalogSource/Subscription/OLM
# channels entirely (a single bundle has no channel to select).
#
...
# - INSTALL_TIMEOUT
# - KUBECONFIG
# - SNAPSHOT (JSON string, used to find the bundle image reference)
# - BUNDLE_COMPONENT_NAME (default: gitops-operator-bundle-main)

# shellcheck source=./lib/wait-for-resources.sh
source "$(dirname "${BASH_SOURCE[0]}")/lib/wait-for-resources.sh"

BUNDLE_COMPONENT_NAME="${BUNDLE_COMPONENT_NAME:-gitops-operator-bundle-main}"

BUNDLE_IMAGE=$(SNAPSHOT="${SNAPSHOT}" BUNDLE_COMPONENT_NAME="${BUNDLE_COMPONENT_NAME}" python3 -c "
import json, os, sys
snapshot = json.loads(os.environ['SNAPSHOT'])
name = os.environ['BUNDLE_COMPONENT_NAME']
for component in snapshot.get('components', []):
    if component.get('name') == name:
        print(component['containerImage'])
        sys.exit(0)
sys.exit(f'ERROR: component {name!r} not found in SNAPSHOT')
")

if [[ -z "${BUNDLE_IMAGE}" ]]; then
  echo "ERROR: Could not resolve bundle image for component ${BUNDLE_COMPONENT_NAME} from SNAPSHOT"
  exit 1
fi

echo "Installing GitOps Operator from bundle: ${BUNDLE_IMAGE}"
...
# 3. Install the operator directly from the bundle image (no CatalogSource/Subscription)
...
echo "Operator installed successfully (CSV: ${CSV_NAME:-unknown})"

# 4. Fallback: inject pull credentials into openshift-gitops namespace
...
# 5. Verify all related images are available at mirrors
echo ""
echo "=========================================="
...
echo "--- CatalogSource(s) / Subscription(s) created by operator-sdk ---"
oc get catalogsource -A -o wide || true
oc get subscription -A -o wide || true
echo ""
...
# 6. Verify default ArgoCD instance is ready
echo ""
echo "=========================================="
...
# 7. Collect cluster-wide debug info (on success)
echo ""
echo "=========================================="
...
# 8. Verify pull-secret propagated to nodes
if [[ -f "/quay-pull-credentials/.dockerconfigjson" ]]; then
  echo ""
  echo "=========================================="
