...
collect_snapshot() {
    ...
    collect_pod_logs_with_tail "${NAMESPACE}" "${snapshot_dir}" 100
}
...
collect_final() {
    ...
    collect_pod_logs "${NAMESPACE}" "${FINAL_DIR}"
}
...
upload_logs() {
    ...
    local tag="${PIPELINE_RUN_NAME}-task-${BRANCH_NAME}"
    local size=$(du -sh "${LOGS_DIR}" 2>/dev/null | cut -f1)

    if oras_push_tarball "${LOGS_DIR}" "${QUAY_REPO}" "${tag}" \
        "application/vnd.konflux.logs.v1+tar" "${tag}" >/dev/null; then
        echo "Sidecar upload #${UPLOAD_COUNT} pushed to ${QUAY_REPO}:${tag} (${size})"
    else
